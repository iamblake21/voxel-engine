...
public class InteractionManager {
    ...
    public RaycastResult performRaycast(Player player, World world, EntityManager entityManager) {
        ...
        // Raycast entities
        RaycastResult entityResult = raycastEntities(entityManager, player, eyeX, eyeY, eyeZ, dirX, dirY, dirZ, ENTITY_REACH);
    ...
    private Direction determineFace(float localX, float localY, float localZ) {
        ...
        if (distBottom < minDist) { minDist = distBottom; face = Direction.DOWN; }
        if (distNorth < minDist) { minDist = distNorth; face = Direction.NORTH; }
        if (distSouth < minDist) { minDist = distSouth; face = Direction.SOUTH; }
        if (distWest < minDist) { minDist = distWest; face = Direction.WEST; }
        if (distEast < minDist) { face = Direction.EAST; }
