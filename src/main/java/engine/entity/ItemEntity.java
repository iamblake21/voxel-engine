...
public class ItemEntity extends Entity {
    ...
    public boolean tryMerge(ItemEntity other) {
        ...
        if (stack == null || other.stack == null)
            return false;
        if (!stack.canStackWith(other.stack))
            return false;
    ...
    protected void saveAdditional(engine.world.item.nbt.NBTTagCompound tag) {
        tag.setInt("Age", age);
        tag.setInt("PickupDelay", pickupDelay);
        if (stack != null) {
            engine.world.item.nbt.NBTTagCompound stackTag = new engine.world.item.nbt.NBTTagCompound();
            stack.save(stackTag);
    ...
    protected void loadAdditional(engine.world.item.nbt.NBTTagCompound tag) {
        age = tag.getInt("Age");
        pickupDelay = tag.getInt("PickupDelay");
        if (tag.hasKey("Item")) {
            stack = ItemStack.load(tag.getTag("Item"));
        }
    }
