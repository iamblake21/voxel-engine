...
public class LivingEntity extends Entity {
    ...
    protected void updateAiMovement() {
        ...
        // Reset movement input (L'AI deve reimpostarlo ogni tick se vuole continuare a
        // muoversi)
        moveForward = 0;
        moveStrafe = 0;
        jumping = false;
    ...
    protected void updateAnimation(float deltaTime) {
        ...
        if (speed > 0.01f) {
            // Speed (m/s) * deltaTime (s) * Multiplier
            // Walk frequency: ~2 steps/sec = ~12.5 rad/sec
            // if speed=4, 4 * 1/60 * M .
            // M=10 -> ~0.66 rad/frame -> 40 rad/sec (TOO FAST)
            // M=3 -> ~0.2 rad/frame -> 12 rad/sec (~2 Hz, Reasonable)
            limbSwing += speed * deltaTime * 2.5f;
            limbSwingAmount = Math.min(1f, limbSwingAmount + deltaTime * 2f);
            currentAnimation = "walk";
        } else {
            limbSwingAmount = Math.max(0f, limbSwingAmount - deltaTime * 2f);
    ...
    protected float prevSwingProgress = 0;
    ...
    protected void updateSwing() {
        this.prevSwingProgress = this.swingProgress;

        // 60 TPS loop -> 15 ticks = 0.25 seconds
        int swingDuration = 15;
        if (swinging) {
            swingProgressInt++;
            if (swingProgressInt >= swingDuration) {
                swingProgressInt = 0;
                swinging = false;
            }
        } else {
        ...
        this.swingProgress = (float) swingProgressInt / (float) swingDuration;
    }

    public float getSwingProgress(float partialTick) {
        float lerp = prevSwingProgress + (swingProgress - prevSwingProgress) * partialTick;
        return lerp;
    }
