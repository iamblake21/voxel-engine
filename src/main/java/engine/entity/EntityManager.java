...
public class EntityManager {
    ...
    private void processItemMerging() {
        ...
        for (int i = 0; i < items.size(); i++) {
            ...
            if (a.isRemoved() || !a.canPickup())
                continue;

            for (int j = i + 1; j < items.size(); j++) {
                ItemEntity b = items.get(j);
                if (b.isRemoved() || !b.canPickup())
                    continue;
    ...
    public void update(float deltaTime) {
        ...
        // CRITICAL: Pre-tick must run at Physics Rate (60Hz), not Game Logic Rate
        // (20Hz)
        // This allows smooth interpolation with the PhysicsEngine
        tickAccumulator += deltaTime;

        while (tickAccumulator >= TICK_TIME) {
            // Tick logic (AI, Scripts, Eventi) - Runs at 20Hz
            for (Entity e : entities) {
                if (!e.isRemoved()) {
                    e.update(TICK_TIME);
                }
        ...
        // partialTick managed manually via setPartialTick from Engine
    ...
    /**
     * Set partial tick from GameLoop alpha.
     */
    public void setPartialTick(float alpha) {
        this.partialTick = alpha;
    }
