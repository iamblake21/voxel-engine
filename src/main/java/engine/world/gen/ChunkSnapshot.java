...
/**
 * Snapshot thread-safe di un'area 3x3 chunk per la generazione asincrona di
 * mesh e luce.
 * Ora include i buffer di luce (Block e Sky) dei vicini per una corretta
 * propagazione cross-chunk.
 */
public class ChunkSnapshot implements WorldAccess {
    ...
    public int getSkyLight(int x, int y, int z) {
        // Controllo bounds rapido (opzionale se ti fidi della BFS, ma sicuro)
        if (x < 0 || x >= chunkSize || z < 0 || z >= chunkSize || y < 0 || y >= chunkHeight) {
            return 15; // Luce esterna default
        }
        // Usa lo stesso array o logica che usi in setSkyLight.
        // Esempio: return this.skyLightData[getIndex(x, y, z)];
        // Se usi nibbles (mezzi byte), la logica sarà simile a Chunk.java
        return getLightValue(this.skyLightBuffer, x, y, z);
    }

    public int getBlockLight(int x, int y, int z) {
        if (x < 0 || x >= chunkSize || z < 0 || z >= chunkSize || y < 0 || y >= chunkHeight) {
            return 0; // Buio esterno default
        }
        return getLightValue(this.blockLightBuffer, x, y, z);
    }

    // Se non hai già un helper per leggere l'array (soprattutto se usi byte[] per
    // risparmiare memoria):
    private int getLightValue(byte[] array, int x, int y, int z) {
        int index = (y * chunkSize * chunkSize) + (z * chunkSize) + x;
        return array[index]; // O la logica dei mezzi-byte se la usi
    }
